...
print('Нажмите на пробел, чтобы начать игру.')
...
class Game:
    ...
    # метод начала игры
    def start_game(self):
        x = screen_surface.get_width() // 2
        y = screen_surface.get_height() // 2 - 75
        y1 = y + screen_surface.get_height() // 6

        # Определяем шрифт и размер текста
        font = pygame.font.Font('freesansbold.ttf', 21)

        # Создаем текстовую поверхность с заданным текстом
        text = font.render("ДЛЯ НАЧАЛА ИГРЫ НАЖМИТЕ", True, (254, 255, 137))
        text1 = font.render("ПРОБЕЛ", True, (254, 255, 137))
        screen_surface.fill(BG)
        # Определяем координаты для центрирования текста
        text_rect = text.get_rect(center=(x, y))
        text1_rect = text1.get_rect(center=(x, y1))

        # Отрисовываем текст на окне
        screen_surface.blit(text, text_rect)
        screen_surface.blit(text1, text1_rect)
        ...
        while waiting:
            for event in pygame.event.get():
                # Если нажата клавиша пробел, закрываем окно и запускаем игру
...
class Bird:
    ...
    def check_collision(self, pipe):
        if (pipe.rect_top.left <= self.rect.right and pipe.rect_top.bottom >= self.rect.top) \
            ...
            game.end_game()
            print('Вы столкнулись с препятствием!')
